...
import { supabaseBrowserClient as supabase, supabaseBrowserClient } from "@utils/supabase/client"
...
// Function to subscribe to real-time updates for a conversation
...
export const subscribeToTypingIndicators = (conversationId: string, callback: (payload: any) => void) => {
    const subscription = supabaseBrowserClient
        ...
        .on(
            ...
            (payload) => {
                if (payload.new && payload.new.typing_at !== payload.old.typing_at) {
                    callback(payload)
                }
            },
        )
...
export const ensureStorageBucket = async (bucketName = "files") => {
    try {
      ...
      if (!bucketExists) {
        ...
        const { error: createError } = await supabaseBrowserClient.storage.createBucket(bucketName, {
          public: true,
