...
const generateMockActivities = (page: number, limit: number) => {
  ...
  const baseMockActivities = [
    ...
    },
    {
      ...
      detail:
        "@Cristal Parker Can you share your thoughts on this design approach?",
  ...
  const generateActivity = (index: number) => {
    ...
    switch (type) {
      case "message":
        content = "sent a message in #design_team";
        detail =
          "Here's the update on the project status. Let me know if you need more details.";
        conversationId = "channel-2";
        break;
      case "file":
        content = "uploaded a file in #marketing_team";
    ...
    if (!initialLoading) {
      const fetchData = async () => {
        await loadMore(async (page, limit) => {
          ...
          return filterActivities(data as any);
        });
      };
      fetchData();
    }
          ...
          {activities.length > 0 ? (
            <>
              ...
              <div ref={loadingRef}>
                ...
                {!hasMore && !isLoading && (
                  <EndOfFeed message="You've reached the end of your activity feed" />
                )}
              </div>
            </>
          ) : (
