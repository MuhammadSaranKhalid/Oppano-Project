...
import { ChevronDown, Eye, EyeOff, AlertCircle, CheckCircle, Loader2, Info, Shield, X, Mail, Clock } from "lucide-react"
  ...
  async function onSubmit(values: SignUpFormValues) {
    try {
      ...
      if (error.code === "rate_limit_exceeded") {
        ...
        setError(error.message || "Too many requests. Please try again in a minute.")
        return
      }
      ...
      if (error.field) {
        ...
        form.setFocus(error.field as "email" | "password" | "name" | "confirmPassword")
      } else {
        // General error
    ...
    if (accountStatus.verified) {
      return (
        <Alert className="border-blue-500 bg-blue-50 text-blue-700 animate-in fade-in-50">
          ...
          <AlertDescription className="flex flex-col gap-2">
            <p>An account with this email address already exists and has been verified.</p>
      ...
      return (
        <Alert className="border-amber-500 bg-amber-50 text-amber-700 animate-in fade-in-50">
          ...
          <AlertDescription className="flex flex-col gap-2">
            <p>
              An account with this email address already exists but hasn't been verified yet. Please check your inbox
      ...
      <div className="w-full max-w-md mx-auto flex-1 flex flex-col items-center justify-center px-4">
        ...
        <Card className="w-full border-[#dadbd8] shadow-sm">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl font-bold text-center text-[#1d1c1d]">Create your account</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            {isRateLimited && (
              <Alert className="border-amber-500 bg-amber-50 text-amber-700 animate-in fade-in-50">
                ...
                <AlertDescription>
                  We're processing too many requests right now. Please wait a moment and try again.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isSuccess && !accountStatus?.exists && (
              <Alert className="border-green-500 bg-green-50 text-green-700 animate-in fade-in-50">
                ...
                <AlertDescription>
                  Please check your email for confirmation instructions. You'll need to verify your email before logging
                  in.
                </AlertDescription>
              </Alert>
            )}
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-[#1d1c1d]">Full Name</FormLabel>
                      <FormControl>
                        <Input
                          placeholder="John Doe"
                          {...field}
                          disabled={isLoading || isSuccess || !!accountStatus?.exists || isRateLimited}
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="w-80 p-3">
                              <div className="space-y-2">
                                <h4 className="font-medium">Your password must include:</h4>
                                <ul className="space-y-1 text-sm">
                                  <li className="flex items-center">
                                    <span
                                      className={`mr-2 ${passwordCriteria.length ? "text-green-500" : "text-red-500"}`}
                                    >
                                  ...
                                  <li className="flex items-center">
                                    <span
                                      className={`mr-2 ${passwordCriteria.uppercase ? "text-green-500" : "text-red-500"}`}
                                    >
                                  ...
                                  <li className="flex items-center">
                                    <span
                                      className={`mr-2 ${passwordCriteria.lowercase ? "text-green-500" : "text-red-500"}`}
                                    >
                                  ...
                                  <li className="flex items-center">
                                    <span
                                      className={`mr-2 ${passwordCriteria.number ? "text-green-500" : "text-red-500"}`}
                                    >
                                  ...
                                  <li className="flex items-center">
                                    <span
                                      className={`mr-2 ${passwordCriteria.special ? "text-green-500" : "text-red-500"}`}
                                    >
                        ...
                        <div className="mt-2 space-y-2">
                          ...
                          <div className="flex flex-wrap gap-1 mt-2">
                            {Object.entries(passwordCriteria).map(([key, met]) => (
                              ...
                              >
                                {met ? <CheckCircle className="h-3 w-3 mr-1" /> : <X className="h-3 w-3 mr-1" />}
                                <span>
                                  {key === "length"
                                    ? "8+ chars"
                                    : key === "uppercase"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-[#1d1c1d]">Confirm Password</FormLabel>
                      <FormControl>
                        <div className="relative">
                          <Input
                ...
                <Button
                  ...
                  disabled={isLoading || isSuccess || isGoogleLoading || !!accountStatus?.exists || isRateLimited}
