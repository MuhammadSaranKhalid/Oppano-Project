...
import { Home, MessageSquareReply, Activity, Clock, Folder, MoreVertical, Settings } from "lucide-react"
import { cn } from "@/lib/utils"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
import { useChatStore } from "@/store/chat-store"

const navItems = [
  { icon: Home, label: "Home", path: "/replies", section: "replies" },
  { icon: Clock, label: "Time", path: "/time", section: "time" },
  { icon: Folder, label: "Files", path: "/files", section: "files" },
  { icon: MessageSquareReply, label: "Replies", path: "/replies", section: "replies" },
  { icon: Activity, label: "Activity", path: "/activity", section: "activity" },
]

const bottomNavItems = [
  { icon: Settings, label: "Settings", path: "/settings", section: "settings" },
  { icon: MoreVertical, label: "More", path: "/more", section: "more" },
]
...
export function IconSidebar() {
  ...
  const getActiveSection = (path: string) => {
    if (path.startsWith("/replies")) return "replies"
        ...
        <div className="flex flex-col items-center gap-7">
          {navItems.map((item) => (
            <Tooltip key={item.path}>
              ...
              <TooltipContent side="right" sideOffset={10} className="bg-gray-800 text-white border-gray-800">
                {item.label}
              </TooltipContent>
            </Tooltip>
          ))}
