...
export function FileUpload({
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      if (e.dataTransfer.items && e.dataTransfer.items.length > 0) {
        ...
        if (hasDirectory) {
          toast({
            ...
            description:
              "Please use the 'Select Folder' button to upload folders",
            variant: "destructive",
          });
          return;
        }
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        ...
        uploadFilesToSupabase(filesToUpload, Array.from(e.dataTransfer.files));
      }
    },
    [processFiles, toast]
  );
  ...
  const handleFileSelect = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
        ...
        uploadFilesToSupabase(filesToUpload, Array.from(e.target.files));
  ...
  const uploadFilesToSupabase = async (
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      const rawFile = rawFiles.find(
        (f) => f.name === fileObj.name || f.name.endsWith(`/${fileObj.name}`)
      );
      ...
      try {
        ...
        const storagePath = `organizations/${organizationId}/${uniqueFileName}`;
    ...
    setFiles((prevFiles) => updateFolderProgress(prevFiles));
    ...
    if (successfulUploads.length > 0) {
      ...
      setFiles((prevFiles) =>
        prevFiles.filter((f) => f.status === "error" || f.isFolder)
      );
    }
  };
    ...
    setFiles((prevFiles) => {
      ...
      if (fileToRemove.parentId) {
        ...
        if (parent && parent.children) {
          ...
          return prevFiles
            .map((f) => (f.id === parent.id ? updatedParent : f))
  ...
  const retryUpload = useCallback((file: UploadedFile) => {
    ...
    setFiles((prevFiles) =>
      prevFiles.map((f) => (f.id === file.id ? retryFile : f))
    );
    // We would need the original File object to retry the upload
    // This is a simplified version
    toast({
      title: "Retry not available",
      description: "Please remove the file and upload it again.",
      variant: "destructive",
    });
  }, []);
  ...
  const renderFileItem = (file: UploadedFile, depth = 0) => {
    ...
    const childFiles = hasChildren
      ? (file
          .children!.map((id) => files.find((f) => f.id === id))
      ...
      <div key={file.id} className="space-y-2">
        ...
        {file.isFolder && isExpanded && sortedChildren.length > 0 && (
          <div className="pl-8 space-y-2">
            {sortedChildren.map((child) => renderFileItem(child, depth + 1))}
          </div>
        )}
      </div>
    );
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-sm text-gray-500 max-w-md">
            Drop your files here or click to browse. You can also upload entire
            folders with their structure preserved.
          </p>
